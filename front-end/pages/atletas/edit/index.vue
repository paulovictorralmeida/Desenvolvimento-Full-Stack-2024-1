<template>
  <div class="card-form-container">
    <UCard>
      <template #header>
        <h3 v-if="isEditPage">Editar Atleta</h3>
        <h3 v-else>Cadastrar Atleta</h3>
      </template>
      ...
      <div class="card-form-input">
        <UInput v-model="idade" placeholder="Idade" @input="handleBirthValidation" />
      </div>
      <div class="card-form-input">
      ...
      <div class="card-form-input">
        <UInput v-model="peso" placeholder="Peso" @input="handleWeightValidation"/>
      </div>
    ...
    async getData() {
      try {
        const data = await $fetch(
          `http://localhost:8081/ufc-back/atletas/${this.id}`,
          {
            method: "GET",
          }
        );
      ...
      if (
        ...
        try {
          const response = await $fetch(
            !this.isEditPage
              ? `http://localhost:8081/ufc-back/atletas`
              : `http://localhost:8081/ufc-back/atletas/${this.id}`,
            {
              method: this.method,
              body: {
